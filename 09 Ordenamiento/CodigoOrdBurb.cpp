...
struct trabajador x[MAX] ;
 ...
 cout<<"Bienvenido al registro de personal \n";
 cout<<"Ingrese el numero de trabajadores que desea registrar:" ;cin>>n ;
 leerpersonal(n,x) ;
 ordenaxfecha(n,x) ;
 listado(n,x) ;
 ...
 cout<<"Ingrese el año:" ;cin>>f.anio ;
 pos=bBinariafecha(n,x,f) ;
 if(pos==-1)cout<<"Lo sentimos, el trabajador no fue encontrado";
 ...
 void leerpersonal(int n,struct trabajador x[]){
 	for(int i=0;i<n;i++){
	cout<<"Ingresa el codigo del trabajador:";cin>>x[i].codigo;
 	...
 	cout<<"Ingresa el año nacimiento:";cin>>x[i].nacimiento.anio;
 	cout<<endl;
 	}
}
